...
Luxury underground parking in densely populated urban areas like Manhattan requires enhanced security, convenience, and safety for vehicle owners. Many existing parking systems lack sufficient access control and collision prevention mechanisms, increasing the risk of unauthorized access and accidents. This underscores the need for a more secure and user-friendly solution that provides peace of mind while ensuring efficient vehicle management.

Our proposed automated underground parking system enhances security, safety, and user convenience. The system utilizes an LDR sensor to detect vehicle presence at the entrance gate, which can be unlocked using a matrix keypad for secure code-based access. Inside the parking area, LED indicators (red and green) guide the driver on when it is safe to enter, while an ultrasonic distance sensor prevents collisions by triggering a buzzer alert when the vehicle is too close to the wall. For a seamless exit, an exit button opens the gate, allowing the driver to back out safely. Additionally, a button inside the parking spot enables the driver to change the matrix keypad code, with updates displayed on the serial monitor for enhanced security control.
...
| Component                | Description                                      |
|--------------------------|--------------------------------------------------|
| **Microcontroller**      | STM32 Nucleo – Core controller for the system    |
| **Matrix Keypad**        | Used for secure code-based entry                 |
| **LDR Sensor**          | Detects vehicle presence at entrance and spot    |
| **Ultrasonic Sensor**   | Measures distance to prevent collisions          |
| **Buzzer (Collision)**  | Triggers when the car is too close to a wall     |
| **Buzzer (Security)**   | Activates on multiple failed code attempts       |
| **Indicator LEDs**      | Blue for engine on, Green for ignition enabled   |
| **Status LED (Red)**    | Indicates gate movement                          |
| **Status LED (Green)**  | Signals safe entry                              |
| **LCD Display**         | Displays system messages                         |
| **Positional Servo**    | Controls gate movement                           |
...
This document provides an overview of the **Automated Luxury Underground Parking System** and outlines its system components, success scenarios, and failure conditions. It ensures a robust, secure, and user-friendly parking solution.
